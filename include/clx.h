...
CLX_INLINE_HOT LValue nil() {
    return LValue();
}

//------------------ Creates a boolean LValue
CLX_INLINE_HOT LValue boolean(bool v) {
    return LValue(v);
}

//------------------ Creates a number LValue
CLX_INLINE_HOT LValue number(double v) {
    return LValue(v);
}

//------------------ Creates an integer LValue
CLX_INLINE_HOT LValue integer(int64_t v) {
    return LValue(v);
}
...
CLX_INLINE_HOT LValue thread(LThread *t) {
    return LValue(ValueType::Thread, t);
}

//------------------ Creates a light userdata LValue
CLX_INLINE_HOT LValue lightuserdata(void *p) {
    return LValue(ValueType::UserData, reinterpret_cast<LHeader *>(p));
}

//------------------ Returns type enum of a value
CLX_INLINE_HOT ValueType type_of(const LValue &v) {
    return v.type;
}

//------------------ Returns type name string from enum
CLX_INLINE_HOT const char *type_name(ValueType t) {
    return VALUE_TYPE_NAMES[static_cast<size_t>(t)];
}

//------------------ Checks if value is nil
CLX_INLINE_HOT bool is_nil(const LValue &v) {
    return v.type == ValueType::Nil;
}

//------------------ Checks if value is boolean
CLX_INLINE_HOT bool is_bool(const LValue &v) {
    return v.type == ValueType::Boolean;
}

//------------------ Checks if value is number or integer
CLX_INLINE_HOT bool is_number(const LValue &v) {
    return v.type == ValueType::Double || v.type == ValueType::Int64;
}

//------------------ Checks if value is integer
CLX_INLINE_HOT bool is_integer(const LValue &v) {
    return v.type == ValueType::Int64;
}

//------------------ Checks if value is string
CLX_INLINE_HOT bool is_string(const LValue &v) {
    return v.type == ValueType::String;
}

//------------------ Checks if value is table
CLX_INLINE_HOT bool is_table(const LValue &v) {
    return v.type == ValueType::Table;
}

//------------------ Checks if value is function
CLX_INLINE_HOT bool is_function(const LValue &v) {
    return v.type == ValueType::Function;
}

//------------------ Checks if value is thread
CLX_INLINE_HOT bool is_thread(const LValue &v) {
    return v.type == ValueType::Thread;
}

//------------------ Checks if value is userdata
CLX_INLINE_HOT bool is_userdata(const LValue &v) {
    return v.type == ValueType::UserData;
}

//------------------ Always false (API compat)
CLX_INLINE_HOT bool is_none(const LValue &v) {
    return false;
}

//------------------ Checks if value is nil
CLX_INLINE_HOT bool is_noneornil(const LValue &v) {
    return v.type == ValueType::Nil;
}
...
CLX_INLINE_COLD LUserdata *check_userdata(LState *L, const LValue &v) {
    if (v.type == ValueType::UserData)
        return static_cast<LUserdata *>(v.as_pointer());
...
CLX_INLINE_COLD LTable *check_table(LState *L, const LValue &v) {
    if (v.type == ValueType::Table)
        return static_cast<LTable *>(v.as_pointer());
...
CLX_INLINE_COLD LCFunction *check_function(LState *L, const LValue &v) {
    if (v.type == ValueType::Function)
        return static_cast<LCFunction *>(v.as_pointer());
...
CLX_INLINE void raw_set_i(LState *L, const LValue &table, int64_t idx, const LValue &val) {
    ...
    if (static_cast<uint64_t>(idx - 1) < t->array_cap) {
        ...
        if (static_cast<size_t>(idx) > t->array_size)
            t->array_size = static_cast<size_t>(idx);
    } else {
...
CLX_INLINE_HOT bool rawequal(const LValue &a, const LValue &b) {
    ...
    if (a.type == ValueType::String)
        return a.val.payload.u64 == b.val.payload.u64;
...
CLX_INLINE MultiValue next(LState *L, const LValue &table, const LValue &key) {
    ...
    if (key.type == ValueType::Nil) {
        ...
        if (idx >= 1 && static_cast<size_t>(idx) <= t->array_size) {
            size_t found = clx_find_first_nonnil(
                reinterpret_cast<const uint8_t *>(t->array_types), t->array_size, static_cast<size_t>(idx));
...
CLX_INLINE_COLD void checktype(LState *L, int argnum, const LValue &v, ValueType t) {
    if (v.type != t) {
        char buf[128];
        std::snprintf(buf, sizeof(buf), "bad argument #%d (%s expected, got %s)", argnum,
            VALUE_TYPE_NAMES[static_cast<size_t>(t)], VALUE_TYPE_NAMES[static_cast<size_t>(v.type)]);
...
CLX_INLINE_COLD void argexpected(
    LState *L, bool cond, int argnum, const LValue &v, const char *wanted_type, const char *extramsg = nullptr) {
    if (!cond) {
        char buf[128];
        const char *actual = VALUE_TYPE_NAMES[static_cast<size_t>(v.type)];
        int n = std::snprintf(buf, sizeof(buf), "bad argument #%d (%s expected, got %s)", argnum, wanted_type, actual);
...
CLX_INLINE_COLD int64_t check_field_integer(LState *L, const LValue &v, const char *field) {
    ...
    char buf[192];
    std::snprintf(buf, sizeof(buf), "field '%s' (integer expected, got %s)", field,
...
CLX_INLINE_COLD double check_field_number(LState *L, const LValue &v, const char *field) {
    ...
    char buf[192];
    std::snprintf(
        buf, sizeof(buf), "field '%s' (number expected, got %s)", field, VALUE_TYPE_NAMES[static_cast<size_t>(v.type)]);
    throw LRuntimeException(LValue(L->intern_string(buf)));
}
...
CLX_INLINE_COLD const char *check_field_string(LState *L, const LValue &v, const char *field) {
    ...
    char buf[192];
    std::snprintf(
        buf, sizeof(buf), "field '%s' (string expected, got %s)", field, VALUE_TYPE_NAMES[static_cast<size_t>(v.type)]);
    throw LRuntimeException(LValue(L->intern_string(buf)));
}
...
//------------------ Throws a type error
[[noreturn]] CLX_INLINE_COLD void type_error(
    LState *L, int argnum, const char *expected, const LValue *args, size_t count) {
    char buf[128];
