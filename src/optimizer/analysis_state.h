...
inline bool is_purely_integer_expr(const ASTContext& ctx, const AnalysisState& state, uint32_t node_idx) {
    ...
    if (n.type == NodeType::BinaryOp) {
        ...
        bool is_int_binop = (bop >= static_cast<int>(BinaryOp::Add) && bop <= static_cast<int>(BinaryOp::Mul))
            ...
            || bop == static_cast<int>(BinaryOp::BitAnd)
            || bop == static_cast<int>(BinaryOp::BitOr)
            || bop == static_cast<int>(BinaryOp::BitXor)
            || bop == static_cast<int>(BinaryOp::Shl)
            || bop == static_cast<int>(BinaryOp::Shr);
        if (is_int_binop)
            return is_purely_integer_expr(ctx, state, n.as.bin_op.left) && is_purely_integer_expr(ctx, state, n.as.bin_op.right);
    }
    if (n.type == NodeType::UnaryOp && (n.as.unary_op.op == static_cast<int>(UnaryOp::Minus) || n.as.unary_op.op == static_cast<int>(UnaryOp::BNot)))
