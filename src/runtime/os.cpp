...
static MultiValue os_getenv(LState* L, const LValue* args, size_t count) {
    ...
    if (!val) return MultiValue(LValue());
...
static MultiValue os_setlocale(LState* L, const LValue* args, size_t count) {
    ...
    if (count >= 2 && args[1].type() != LType::Nil) {
        ...
        else if (std::strcmp(cat, "ctype") == 0) category = LC_CTYPE;
        else if (std::strcmp(cat, "monetary") == 0) category = LC_MONETARY;
        else if (std::strcmp(cat, "numeric") == 0) category = LC_NUMERIC;
        else if (std::strcmp(cat, "time") == 0) category = LC_TIME;
        else {
            char buf[128];
    ...
    if (!result) return MultiValue(LValue());
