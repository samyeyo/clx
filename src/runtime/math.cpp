...
static MultiValue math_abs(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'abs' (number expected, got no value)");
...
static MultiValue math_acos(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'acos' (number expected, got no value)");
...
static MultiValue math_asin(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'asin' (number expected, got no value)");
...
static MultiValue math_atan(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'atan' (number expected, got no value)");
...
static MultiValue math_ceil(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'ceil' (number expected, got no value)");
    if (clx::is_integer(args[0])) return MultiValue(args[0]);
...
static MultiValue math_cos(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'cos' (number expected, got no value)");
...
static MultiValue math_deg(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'deg' (number expected, got no value)");
...
static MultiValue math_exp(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'exp' (number expected, got no value)");
...
static MultiValue math_floor(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'floor' (number expected, got no value)");
    if (clx::is_integer(args[0])) return MultiValue(args[0]);
...
static MultiValue math_fmod(LState* L, const LValue* args, size_t count) {
    if (count < 2) clx::error(L, "bad argument #2 to 'fmod' (number expected, got no value)");
...
static MultiValue math_max(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'max' (number expected, got no value)");
    LValue m = args[0];
    for (size_t i = 1; i < count; ++i)
        if ((m < args[i]).as_bool()) m = args[i];
    return MultiValue(m);
}
...
static MultiValue math_min(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'min' (number expected, got no value)");
    LValue m = args[0];
    for (size_t i = 1; i < count; ++i)
        if ((args[i] < m).as_bool()) m = args[i];
    return MultiValue(m);
}
...
static MultiValue math_rad(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'rad' (number expected, got no value)");
...
static MultiValue math_sin(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'sin' (number expected, got no value)");
...
static MultiValue math_sqrt(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'sqrt' (number expected, got no value)");
...
static MultiValue math_tan(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'tan' (number expected, got no value)");
...
static MultiValue math_ldexp(LState* L, const LValue* args, size_t count) {
    if (count < 2) clx::error(L, "bad argument #2 to 'ldexp' (number expected, got no value)");
...
static MultiValue math_log(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'log' (number expected, got no value)");
    double x = clx::check_number(L, args[0]);
    if (count > 1) {
...
static MultiValue math_modf(LState* L, const LValue* args, size_t count) {
    if (count == 0) clx::error(L, "bad argument #1 to 'modf' (number expected, got no value)");
    double x = clx::check_number(L, args[0]);
    double intpart;
...
static MultiValue math_randomseed(LState* L, const LValue* args, size_t count) {
    ...
    if (count == 0) {
        ...
        return MultiValue({clx::integer(static_cast<int64_t>(seed >> 32)), clx::integer(static_cast<int64_t>(seed & 0xFFFFFFFFULL))});
    }
...
static MultiValue math_ult(LState* L, const LValue* args, size_t count) {
    if (count < 2) clx::error(L, "bad argument #2 to 'ult' (number expected, got no value)");
