...
void Lexer::advance() {
    while (pos < src.length()) {
        ...
        if (src[pos] == '-' && pos + 1 < src.length() && src[pos + 1] == '-') {
            ...
            if (pos < src.length() && src[pos] == '[') {
                size_t eq = 0;
                while (pos + eq + 1 < src.length() && src[pos + 1 + eq] == '=')
                    eq++;
    ...
    switch (c) {
    case '+':
        current_token = { TokPlus, "+", 0, current_line };
        return;
    case '-':
        current_token = { TokMinus, "-", 0, current_line };
        return;
    case '*':
        current_token = { TokStar, "*", 0, current_line };
        return;
    case '/':
        if (pos < src.length() && src[pos] == '/') {
            pos++;
            current_token = { TokFloorDiv, "//", 0, current_line };
        } else {
            current_token = { TokSlash, "/", 0, current_line };
        }
        return;
    case '(':
        current_token = { TokLParen, "(", 0, current_line };
        return;
    case ')':
        current_token = { TokRParen, ")", 0, current_line };
        return;
    case '{':
        current_token = { TokLBrace, "{", 0, current_line };
        return;
    case '}':
        current_token = { TokRBrace, "}", 0, current_line };
        return;
    case '[': {
        size_t eq = 0;
        while (pos + eq < src.length() && src[pos + eq] == '=')
            eq++;
        ...
        return;
    case ']':
        current_token = { TokRBracket, "]", 0, current_line };
        return;
    case ':':
        if (pos < src.length() && src[pos] == ':') {
            pos++;
            current_token = { TokDoubleColon, "::", 0, current_line };
        } else {
            current_token = { TokColon, ":", 0, current_line };
        }
        return;
    case '#':
        current_token = { TokLen, "#", 0, current_line };
        return;
    case '.':
        if (pos + 1 < src.length() && src[pos] == '.' && src[pos + 1] == '.') {
            pos += 2;
            current_token = { TokVararg, "...", 0, current_line };
            ...
            current_token = { TokDot, ".", 0, current_line };
        }
        return;
    case ',':
        current_token = { TokComma, ",", 0, current_line };
        return;
    case ';':
        current_token = { TokSemicolon, ";", 0, current_line };
        return;
    case '=':
        if (pos < src.length() && src[pos] == '=') {
            pos++;
            current_token = { TokEqEq, "==", 0, current_line };
        } else {
            current_token = { TokAssign, "=", 0, current_line };
        }
        return;
    case '<':
        if (pos < src.length() && src[pos] == '<') {
            pos++;
            current_token = { TokShl, "<<", 0, current_line };
        } else if (pos < src.length() && src[pos] == '=') {
            pos++;
            current_token = { TokLessEq, "<=", 0, current_line };
        } else {
            current_token = { TokLess, "<", 0, current_line };
        }
        return;
    case '>':
        if (pos < src.length() && src[pos] == '>') {
            pos++;
            current_token = { TokShr, ">>", 0, current_line };
        } else if (pos < src.length() && src[pos] == '=') {
            pos++;
            current_token = { TokGreaterEq, ">=", 0, current_line };
        } else {
            current_token = { TokGreater, ">", 0, current_line };
        }
        return;
    case '~':
        if (pos < src.length() && src[pos] == '=') {
            pos++;
            current_token = { TokNotEq, "~=", 0, current_line };
        } else
            current_token = { TokBitXor, "~", 0, current_line };
        return;
    case '%':
        current_token = { TokMod, "%", 0, current_line };
        return;
    case '^':
        current_token = { TokPow, "^", 0, current_line };
        return;
    case '&':
        current_token = { TokBitAnd, "&", 0, current_line };
        return;
    case '|':
